...
import static org.example.service.UtilService.getIntegerInput;

public class CartService {

    private static final CloseableHttpClient httpClient = HttpClients.createDefault();

    public static void addToCart(String jwt) throws IOException, ParseException {

        int productId = getIntegerInput("Enter the ID of the fruit to add to cart: ");
        int quantity = getIntegerInput("Enter the quantity: ");


        String url = String.format("http://localhost:8080/Cart/addtocart/%d/%d", productId, 1);
    ...
    public static void removeCartItemById(String jwt) throws IOException, ParseException {
        List<CartItem> cartItems = getAllCartItems(jwt);


        int cartItemIdToRemove = getIntegerInput("Enter the Cart ID to remove: ");

        String url = String.format("http://localhost:8080/Cart/cart/removeCartItem/%d", cartItemIdToRemove);
        HttpDelete request = new HttpDelete(url);
        request.setHeader(HttpHeaders.AUTHORIZATION, "Bearer " + jwt);
        CloseableHttpResponse response = httpClient.execute(request);
        System.out.println("Response Code: " + response.getCode());
